...
**会导致什么错误**：
- 恶意插件可以读取 `~/.javaclaw/config.yaml` 中的所有 API Key 和敏感数据
- 恶意插件可以通过 `Runtime.exec()` 执行任意系统命令，等同于完全控制宿主机
...
- 用户取消请求或工具执行超时时，Flux 的 `dispose()` 无法传播到 Virtual Thread 的 `interrupt()`，导致 Agent Loop 继续执行已取消的任务，浪费 LLM token 或请求挂起
...
**会导致什么错误**：
- `@RefreshScope` 需要 Actuator 的 `/refresh` 端点触发，本地文件改了不会自动生效，用户改完配置以为生效了实际没有
...
- 没有熔断：模型 API 宕机时，每个请求都要经历"重试 → 全部超时 → 降级"的完整流程，响应时间暴增
- 没有限流联动：不知道当前模型的 RPM/TPM 消耗，等到收到 429（Too Many Requests）才降级，白白浪费一次请求的等待时间
